...
fn main() {
    ...
    if let Some(date) = matches.value_of("transform") {
        ...
        if re.is_match(date) {
            ...
            let year = splitted_date[2].parse::<i32>().unwrap_or_else(|_| {
                println!("Error while parsing year");
                process::exit(1_i32);
            });
            let month = splitted_date[1].parse::<u32>().unwrap_or_else(|_| {
                println!("Error while parsing month");
                process::exit(1_i32);
            });
            let day = splitted_date[0].parse::<u32>().unwrap_or_else(|_| {
    ...
    let balanced_ternary_flag = matches.is_present("balanced-ternary");
...
fn mark(
    ...
    if date_tuple.2 == day && show_day {
        ...
        if balanced_ternary {
            return format!("{}", int_to_bal_ternary(date_tuple.2 as i128));
        } else {
